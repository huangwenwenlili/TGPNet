...
class SEN12MSCRDataset(data.Dataset):
    #初始化 获得所有图片列表
    def __init__(self,base_dir,scale=2000,data_augmentation=False,random_crop=False):
    ...
    def get_patch(self, season, scene_id, patch_id, bands, iscloudimg):
        ...
        if not iscloudimg:
            ...
            patch_path = os.path.join(self.base_dir, season, scene, filename)
            #print(patch_path)
        self.patch_path_out = patch_path
    ...
    def get_path_path(self):
       return self.patch_path_out
    ...
    def getsen12ms_item(self, img_index):
        s1img, s2img, s2cldimg, _ = self.get_triplets(img_index[0], img_index[1], img_index[2])
        s1img = s1img.squeeze(0)
        s2img = s2img.squeeze(0)
        s2cldimg = s2cldimg.squeeze(0)
        if self.data_augmentation:
            self.augment_rotation_param = np.random.randint(0, 4)
            self.augment_flip_param = np.random.randint(0, 3)

        if self.random_crop:
            self.random_crop_paramx = np.random.randint(0, self.crop_size)
            self.random_crop_paramy = np.random.randint(0, self.crop_size)

        if self.data_augmentation:
            if not self.augment_flip_param == 0:
                s2img = np.flip(s2img, self.augment_flip_param)
                s2cldimg = np.flip(s2cldimg, self.augment_flip_param)
            if not self.augment_rotation_param == 0:
                s2img = np.rot90(s2img, self.augment_rotation_param, (1, 2))
                s2cldimg = np.rot90(s2cldimg, self.augment_rotation_param, (1, 2))
        ...
        s2CSMimg = get_cloud_cloudshadow_mask(s2cldimg, 0.2)
        s2CSMimg[s2CSMimg != 0] = 1
        ...
        s1img[0, :, :] = ((np.clip(s1img[0, :, :], -25.0, 0.0) + 25.0) / (25)) * 2
        s1img[1, :, :] = ((np.clip(s1img[1, :, :], -35.0, 0.0) + 35.0) / (35)) * 2

        inputdata = np.concatenate((s1img, s2cldimg), axis=0)
        truthdata = np.concatenate((s1img, s2img), axis=0)
        # 返回三个数据 输入神经网络的数据（15,256,256） Cloud and shadow mask（csm 1*256*256） 无云图片（target 13*256*256）
        # inputdata=s2cldimg
        return inputdata, s2CSMimg, truthdata, self.patch_path_out
    ...
    def __getitem__(self,index):
        ...
        #返回三个数据 输入神经网络的数据（15,256,256） Cloud and shadow mask（csm 1*256*256） 无云图片（target 13*256*256）
        # inputdata=s2cldimg
        return inputdata,s2CSMimg,s2img,self.patch_path_out
